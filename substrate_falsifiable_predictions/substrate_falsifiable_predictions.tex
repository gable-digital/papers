...
of several independent observables. Table~\ref{tab:cross-anchor}
makes this explicit: each row uses a single observable as the sole
input and computes every other observable from the framework's
structural relations. Agreement across rows tests internal
consistency; disagreement against the observed values is a
prediction error of the framework. The fine-structure constant
$\alpha_{\rm EM}$ is included even though it is dimensionless and
does not pin the mass scale --- the $\alpha_{\rm EM}$ column shows
the framework's single structural prediction (lepton-VP-corrected
$\alpha_2 \sin^2\theta_W$, $\sim\!877$~ppm above CODATA, with
hadronic VP not yet substrate-derived; J7).
...
A complete observable-by-anchor cross-tabulation covering all 102
predicted observables (every entry in the framework's structural
catalogue --- 42 particle-mass entries plus electroweak couplings,
mixing angles, CP phases, gauge bosons, lifetimes, decay widths,
Compton wavelengths, atomic constants, Yukawa couplings, dark-matter
candidate spectrum, and gravitational sector) is provided as
\href{https://github.com/gable-digital/papers/blob/main/substrate_falsifiable_predictions/cross_anchor_full.csv}{\texttt{cross\_anchor\_full.csv}}
alongside this paper. The CSV adds a seventh, parameter-free anchor
not shown in Table~\ref{tab:cross-anchor}: the framework's
structurally-derived Higgs vev (\texttt{vC\_struct\_anchor}, $v_C$
computed from $E_8$ invariants alone with no PDG input). For each
observable each anchor column carries the predicted value and the
relative deviation in ppm against CODATA/PDG, all computed at
$\mathrm{mp.prec}=500$.
