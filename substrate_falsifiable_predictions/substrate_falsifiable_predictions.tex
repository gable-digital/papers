...
\noindent\textbf{Internal topology-discrimination test (4-tier).}
A measurement-level discrimination test against the leading
alternative scenario (Tian--Yau $\Z{3}$) is reported as a
four-tier table to expose filter dependence: \emph{conservative
(full $20{+}20$ seed sample, no filter): $|t| = 3.99\sigma$,
percentile 95\% CI $[3.29, 10.64]$, BCa $[2.89, 8.97]$ — CI lower
bound straddles $5\sigma$; strict-converged
(residual${<}\text{tol}$ AND iters${<}\text{cap}$, $n_{\rm Sc}=16$):
$|t| = 6.92\sigma$, BCa $[5.30, 9.04]$, conditional on the
convergence filter which preferentially trims $4/20$ Schoen seeds
from the upper tail.} The conservative tier is the referee-safe
unconditional reading. Source:
\verb|output/p5_10_n40k_p5_5k.log:75-78|.
