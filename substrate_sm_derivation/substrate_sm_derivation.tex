...
\section{Computed 4D effective Lagrangian: $\kappa=0$ critical-path milestone}
\label{sec:stage4-lagrangian}

This section reports the first end-to-end computed assembly of the 4D
effective Lagrangian under the rank-4 $SU(4)$ Braun--He--Ovrut--Pantev
(BHOP-2005~\cite{BHOP2005a,BHOP2005b}) bundle on the Schoen
$\mathbb{Z}_3 \times \mathbb{Z}_3$ quotient. The output is the
critical-path completion of the Stage~4 dimensional-reduction skeleton
described in the companion paper, with three sectors carrying
\emph{physically meaningful} structural predictions (kinetic, gauge,
Higgs potential, plus the Newton's-$G$ closure), and the Yukawa,
CKM, and cosmological-constant sectors carrying explicit
$\kappa=0$ \emph{placeholder} content. Honest framing is supplied
sector by sector.
...
\textbf{Status} marks each sector as \emph{physically meaningful}
(structural prediction) or $\kappa=0$ \emph{placeholder} (do not cite
as a physics prediction).}
\label{tab:stage4-lagrangian-summary}
\begin{tabular}{lll}
\toprule
Sector & Status & Headline numerical content \\
\midrule
Kinetic            & meaningful  & 12 chiral fermions; $H^1$ dim $= 48$; canonical normalisation \\
Gauge              & meaningful  & $g_1 = 0.461228$, $g_2 = 0.65096$, $g_3 = 1.2123$ at $M_Z$ \\
Higgs $V(H)$       & meaningful  & $m_H = 125.1998$ GeV; $v_C = 246.21965$ GeV; $\lambda = 0.12928$; $\mu^2 = 7837.49$ GeV$^2$ \\
Newton $G$         & meaningful  & $G_{\rm rec} = 6.67443 \times 10^{-11}$ vs CODATA $6.6743 \times 10^{-11}$ ($\sim 19$~ppm) \\
Yukawa             & $\kappa=0$ placeholder & $Y_u = Y_d = \mathrm{diag}(2267.25, 2358.93, 2200.59)$~GeV (TeV-scale; non-physical) \\
$Y_e, Y_\nu$       & deferred    & $e^c, \nu^c$ modes absent at degree-1 $H^1$ of rank-4 shadow \\
CKM                & $\kappa=0$ placeholder & $V_{\rm CKM} = \mathbb{1}_3$; Wolfenstein $\lambda = A = \bar\rho = \bar\eta = J = 0$ \\
$\Lambda_{\rm cc}$ & $\kappa=0$ placeholder & $V_{\min} = -1.188 \times 10^{8}$~GeV$^4 = -3.38 \times 10^{-66}\,(M_{\rm Pl}^{\rm red})^4$ (56.5 orders too large) \\
\bottomrule
\end{tabular}
\end{table}
...
\qquad
G_{\rm CODATA-2022} \;=\; 6.6743 \times 10^{-11}~\mathrm{N\cdot m^2/kg^2},
\]
...
\subsection{Yukawa sector: $\kappa=0$ placeholder, NOT a physics prediction}
\label{subsec:stage4-yukawa-placeholder}

The Yukawa matrices computed by the assembly binary at $\kappa=0$ are
\[
Y_u \;=\; Y_d \;=\;
\mathrm{diag}\bigl(2267.25,\; 2358.93,\; 2200.59\bigr)~\mathrm{GeV},
\qquad
V_{\rm CKM} \;=\; \mathbb{1}_3,
\]
with the corresponding diagonalised mass eigenvalues (descending,
GeV) $[1668.02,\; 1603.19,\; 1556.05]$ for both up- and down-type
quarks. The charged-lepton and neutrino blocks $Y_e$ and $Y_\nu$ are
\emph{absent}: the $e^c$ and $\nu^c$ slots carry zero modes in the
degree-1 $H^1(\widetilde X/\Gamma, V_{\rm BHOP})$ catalogue of the
rank-4 shadow bundle (irrep counts: $Q$, $U^c$, $D^c$, $L$ each $12$;
$E^c$, $\nu^c$, $H_u$, $H_d$ each $0$).

\textbf{Honest caveat (load-bearing).}  These values are NOT physical
predictions. They reflect the $\kappa=0$ block-diagonal limit in
which the rank-4 $SU(4)$ BHOP bundle $V$ is treated as a sum of four
line bundles with the off-diagonal $\mathrm{Ext}^1(V_2, V_1)$ data
\emph{not engaged}. The genuine $SU(4)$ extension data is encoded in
the BHOP-2005~\S{}6.5 monad presentation but is not numerically
published in a form that can be consumed by a triple-overlap
integral; the resulting $\kappa=0$ Yukawa structure produces
$\mathcal{O}(\mathrm{TeV})$ values where physics requires the
hierarchical pattern $m_t \approx 173$~GeV, $m_b \approx 4$~GeV,
$m_u \approx 2$~MeV. The frame-collapse convention used inside the
binary is engineered to produce non-zero output (so that the
downstream pipeline is exercised end-to-end), \emph{not} derived
from physics. Likewise, the $Y_e \equiv 0$ and $Y_\nu \equiv 0$
entries are NOT a prediction of massless leptons --- they are a
``no eigenmode in this slot at this cohomology degree'' sentinel,
deferred to higher-degree $H^1$ exploration. Engagement of
$\mathrm{Ext}^1$ ($\kappa \neq 0$) and lift to higher-degree $H^1$
for the $e^c, \nu^c$ sectors are the next critical-path work items.
The CKM matrix is the $3 \times 3$ identity at $\kappa=0$ for the
same reason; Wolfenstein $\lambda = A = \bar\rho = \bar\eta = \delta_{\rm CKM}
= J = 0$.
...
\subsection{Milestone interpretation}
\label{subsec:stage4-milestone}

This section records the first end-to-end \emph{computed} 4D
Lagrangian assembly under the BHOP-2005 rank-4 $SU(4)$ bundle: an
eigenmode catalogue ($n_{\rm pts}=432$, BHOP $H^1$ dim $48$) feeds a
Yukawa-overlap pipeline that produces the $\kappa=0$ Yukawa matrices
above, which in turn assemble the canonical-form Lagrangian of
Eq.~\eqref{eq:lagrangian-canonical} with full SHA-pinned provenance.
The kinetic, gauge, Higgs-potential, and Newton's-$G$ sectors are
\emph{physically meaningful} and reproduce the structural predictions
already documented in earlier sections of this paper. The Yukawa,
CKM, and cosmological-constant sectors carry explicit $\kappa=0$
placeholder status with caveats stated above. The
present milestone is therefore a pipeline-readiness deliverable:
the next critical-path step is to engage $\mathrm{Ext}^1(V_2, V_1)$
($\kappa \neq 0$) inside the BHOP MonadAdapter so that the genuine
$SU(4)$ Yukawa structure is computed; the
hierarchical-Yukawa observables become unblocked at that step.
