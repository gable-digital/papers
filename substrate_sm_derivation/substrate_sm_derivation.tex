...
maintained as
\texttt{book/scripts/cy3\_substrate\_discrimination/rust\_solver/output/p\_grounding\_channels\_v3.json}
with the v4 verdict overlay in
\texttt{book/scripts/cy3\_substrate\_discrimination/references/p\_grounding\_channels\_v4\_results.md};
...
\texttt{p\_phase4\_algebraic\_form\_match.md}, and
\texttt{p\_phase5\_substrate\_native\_derivation.md} in the same
\texttt{references/} directory.

\subsection{The 74-row grounding channel matrix}
...
half-integer dark slots, gravitational-sector outputs, and the open
cosmological rows -- is enumerated as a single matrix entry. Each entry
carries a verdict, the layer of the framework's generating principle
under which it is (or is not) forced, and a citation key into the
relevant Phase report. Verdict counts in v4 are:

\begin{table}[h]
\centering
\caption{v4 verdict distribution across the 74-row grounding channel
matrix. \textsc{grounded-via-*} verdicts cite a substrate-internal or
externally-anchored Rust witness; \textsc{conditional-possibility}
flags structurally-allowed slots the framework does \emph{not} predict
to be occupied.}
...
Verdict & Count \\
\midrule
\textsc{grounded-via-rust}        & 6 \\
\textsc{grounded-via-affine-e8}   & 10 \\
\textsc{grounded-via-wilson-line} & 6 \\
\textsc{grounded-via-icosian}     & 2 \\
\textsc{grounded-via-codata}      & 12 \\
\textsc{grounded-conditional}     & 10 \\
\textsc{conditional-possibility}  & 14 \\
\textsc{partial}                  & 2 \\
\textsc{suggestive}               & 2 \\
\textsc{no-witness-yet}           & 2 \\
\textsc{no-match-phenomenological}& 7 \\
\textsc{falsified}                & 1 \\
\midrule
...
Forty-six of the seventy-four rows carry a \textsc{grounded-*} verdict;
the remaining twenty-eight either remain phenomenological after
exhaustive search (seven), are externally anchored to CODATA (twelve),
flag structurally-allowed but unpredicted slots (fourteen), or fall in
the residual \textsc{partial}/\textsc{suggestive}/\textsc{no-witness-yet}/\textsc{falsified}
categories (five total).
...
\subsection{The 3-piece unified generating principle}
\label{sec:ugp}
...
(icosian sector) rather than $B = h_{SU(3)}$ (colour). Phase 3C
exhaustively confirmed that $k_v = 1132/30$ is BESPOKE: across
$5.77 \times 10^{6}$ new cells over 16 additional structure classes
(OEIS, class numbers, modular tensor category q-dim, etc.) on top of
Phase 2 Stream D's 12 vectors, no published mathematical structure
reproduces this specific rational without a free fitting input. The
JSON field \texttt{k\_v\_exhaustively\_confirmed: true} records this.
\item \textbf{quark $b$-multiset $\{4, 1, 12, 13/2, 17/2, 17/2\}$} --
NOT a single $W(D_8)$ orbit (Phase 2.2 NOT-FOUND); STRUCTURALLY
BESPOKE.
\end{itemize}
...
After the v4 grounding overlay and the Phase 5 substrate-native
re-grounding, the framework's mass-and-coupling table still contains
two distinct kinds of residue.

\paragraph{Bespoke per-particle parameterisations (14 rows).} These
are framework inputs that select which mode pattern each Standard
Model particle IS, and which the unified generating principle does
\emph{not} force:
\begin{itemize}
\item the quark $b$-multiset $\{4, 1, 12, 13/2, 17/2, 17/2\}$, which
fails to be a single $W(D_8)$ orbit (Phase 2.2);
\item the chain position $k_v = 1132/30$ for the Higgs vev, exhaustively
confirmed BESPOKE after $5.77 \times 10^{6}$ structural-class cells
(Phase 3C) on top of $\sim 1.6 \times 10^{9}$ cells across all
streams;
\item the per-lepton 1-bit chirality selection within Wilson-line-forced
planes ($\{11, 17\}$ vs.\ $\{19, 13\}$);
\item per-quark $(N, b, \chi, \lambda)$ assignments;
\item the $W$-boson chain position $k = 34.5$ and Higgs-boson position
$k_v - 2$ (the second is parametrically tied to $k_v$ but inherits
its bespoke status).
\end{itemize}
The framework's \textsc{no-match-phenomenological} verdict for these
rows is honest: the search depth is sufficient to assert that no
\emph{published} mathematical structure forces the stated values
without freely-chosen inputs. The label is not ``unfalsifiable'' --
fitting one different structure that reproduces the same pattern with
fewer free choices would replace these rows.
...
Within the framework's commitments and at the search depth audited:
...
Coxeter-plane decomposition, and icosian closure $B = 2\varphi$ for
vacuum-tension modes -- grounds 46 of 74 rows of the framework's
mass-and-coupling table to ppm-or-better precision via inverse
Sugawara shifts on $\widehat{e}_8$, with substrate-primitive
small-parameter linearisations (Phase 5) replacing the borrowed
heterotic-CY3 wave-function-overlap interpretation. Twenty-eight rows
remain phenomenological, externally CODATA-anchored, structurally
allowed but unpredicted, or in the residual partial / no-witness /
falsified categories. The framework's predictions and falsification
targets are restricted to the unconditional rows; the conditional
half-integer dark slots are explicitly excluded from
no-particle-detected falsification.
