...
\paragraph{2 ABSENT ingredients (committed but not derived).}
\begin{enumerate}
\item Bianchi identity $dH = \mathrm{tr}(R \wedge R) - \mathrm{tr}(F \wedge F)$
-- the Green--Schwarz mechanism is invoked at the
anomaly-cancellation stage, but the $H$-flux Bianchi identity itself
is not derived from substrate primitives.
\item Modular invariance under $SL(2, \mathbb{Z})$ / equivalently the
even self-dual rank-16 lattice for the gauge degrees of freedom -- the
two such lattices ($E_8 \oplus E_8$ and the
$\mathrm{Spin}(32)/\mathbb{Z}_2$ root lattice) are committed; the
modular-invariance constraint that singles them out is not derived.
\end{enumerate}
...
empirical input. The SI unit system itself is substrate-derived
(\S\ref{sec:si-from-substrate}): $c$, $\hbar$, and $e$ are substrate
primitives, and the Cs-133 hyperfine frequency that defines the SI
second is substrate-derivable from the framework's predicted electron
mass, fine-structure constant, and Cs-133 atomic structure. All
``empirical rods'' used to anchor framework predictions -- the CODATA
electron mass, the LEP-measured Higgs vev, the PDG-tabulated quark
masses, the measured CKM and PMNS angles -- are themselves
...
The framework is therefore strictly more predictive than the Standard
Model in the mass sector: where the Standard Model treats the fermion
masses, the Higgs vev, the CKM and PMNS mixing angles, and the gauge
couplings as roughly twenty-five independent numerical inputs, the
substrate framework predicts all of them from substrate primitives
together with an SI unit system that is itself substrate-derived. The
explicit forcing chain is:
...
\textbf{(Residue.)} The remaining \textsc{interpretive} residue is two
1-bit topology choices (lepton-sector $\varphi$-vs-$\tau\varphi$
plane assignment, muon $\eta$ NLO sign) and approximately twenty-five
per-particle assignments where multiple substrate-natural readings
exist but no single rule privileges one. These do not affect the
absolute mass scale or the dimensionless coupling spectrum; they
affect only which specific Coxeter slots within Wilson-line-forced
planes host specific SM particles. The framework's predictions and
falsification targets are restricted to the unconditional rows; the
fourteen conditional half-integer dark slots are explicitly excluded
from no-particle-detected falsification.
