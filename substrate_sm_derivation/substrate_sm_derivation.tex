...
\title{Zero-anchor structural construction of Standard Model observables, a dark-matter tower,
...
This paper presents a framework that structurally constructs all
twenty-six SM parameters, plus Newton's gravitational constant, the
Planck mass, and a discrete seven-mode dark-matter mass spectrum
together with a misalignment-style cosmological abundance estimate,
from the algebraic data of $E_8 \times E_8$ heterotic compactification
on a Schoen Calabi--Yau threefold quotient $X / (\mathbb{Z}_3 \times
\mathbb{Z}_3)$ alone, with no measured mass entering the predictive
path. The Higgs vacuum expectation value is computed in closed form
from the $E_8$ Coxeter invariants $\{\dim, h, m_{\max}, e_6\}$ via
$v_C = (\dim - 2)/(1 - \delta_W) \cdot (1 + 5\,\alpha_\tau^3/8) =
246.21965$~GeV, where $\delta_W = 1/((h+m_{\max})e_6) = 1/1121$ and
$\alpha_\tau = 1/(\dim+h) = 1/278$ are derived in closed form from
those invariants; the gateway tension scale is $h_{\rm eff} = v_C /
R_v$, and every charged-fermion, EW-boson, neutrino, and dark-matter
mass equals the corresponding chain-specific structural ratio times
$h_{\rm eff}$. The mass-spectrum construction introduces no fitted
continuous parameters: every structural integer or discrete
coefficient is either fixed directly by the algebra of $E_8$, the
geometry of the Schoen quotient, the McKay correspondence between
$E_8$ and the binary icosahedral group, or adopted as a framework
assignment built from those structures. As a diagnostic consistency
check on the framework's internal arithmetic, alternatively anchoring
$h_{\rm eff}$ on the CODATA electron or muon mass produces predictions
that agree at \SI{0.0011}{ppm} across the entire spectrum; this
diagnostic is not the predictive path.
...
\begin{equation}
E_8 \;\supset\; SO(10) \times SU(4) \;\supset\;
SU(5) \times U(1) \;\supset\; G_{\rm SM} \times U(1)_X,
\end{equation}
with the final descent realised by Wilson lines along generators
of $\pi_1(X/\Gamma) = \mathbb{Z}_3 \times \mathbb{Z}_3$. The
intermediate $SU(5)$ rank-4 GUT structure is the natural host
for the rank-4 bundle $V$ implied by the three-generation
condition $|c_3(V_{\rm cover})|/(2|\Gamma|) = 3$. The bundle
$V$ committed here is the rank-4 $SU(4)$ extension construction
of Braun--He--Ovrut--Pantev~\cite{BHOPVectorBundle2006} \S 6,
not the wave-28-era rank-3 $SU(3)$ placeholder retracted in the
audit at \texttt{p\_stage3\_bundle\_physics\_audit.md}. The
extension data delivers $|c_3(V_{\rm cover})| = 54$ on the cover
(three generations on the quotient via Atiyah--Singer), the full
heterotic Bianchi anomaly cancellation
$c_2(TX) - c_2(V_{\rm BHOP}) - c_2(V'_{\rm BHOP}) = 6\,\tau_1^2$
(BHOP Eq.~99) with a positive-tension 5-brane wrapping $\tau_1^2$,
Mumford--Takemoto polystability on the documented K\"ahler region
$\{t_a \in [0.25, 8.0],\; t_a/t_b \in [0.25, 4.0]\}$, and a
$3{:}3{:}3$ decomposition of $H^1(X/\Gamma, V_{\rm BHOP})$ into
three copies of the $\mathbf{16}$ of $SO(10)$ under the
$\mathbb{Z}_3 \times \mathbb{Z}_3$ Wilson-line characters; the
right-handed neutrinos arise automatically as the singlet
$\mathbf{1}$ in $\mathbf{16} = \mathbf{10} + \bar{\mathbf{5}} + \mathbf{1}$.
The anomaly-cancellation and polystability checks are
programmatically verified upstream of this paper at commits
\texttt{a3317377} and \texttt{a4d1bc72} respectively. Alternative
chains (the Donagi--Ovrut $SU(5)$-structure-group route through
$E_8 \supset SU(5)$, or $E_8 \supset SO(16)$) admit consistent
compactifications but are not committed here.

\paragraph{Substrate-physical reading of the rank-4 internal symmetry.}
The rank-4 $SU(4)$ structure group of $V$ admits a clean
substrate-physical interpretation in the spirit of the
Pati--Salam $SU(4)_C$ unification of colour and lepton
number~\cite{PatiSalam1974}: the four internal modes of the
bundle correspond to the three colour modes of the
Standard-Model quark sector plus a fourth ``lepton-number''
mode that unifies leptons with quarks under a single
substrate-level 4-tuple. Lepton number is treated as the fourth
colour at the substrate level, with the chiral generation index
threaded through the $\mathbb{Z}_3 \times \mathbb{Z}_3$ Wilson
characters acting on $H^1(X/\Gamma, V_{\rm BHOP})$. This is the
substrate-physical reason the $SU(4)$-structure-group bundle is
preferred over the wave-28 $SU(3)$ placeholder: the framework's
internal-mode-count for one mode-pattern category includes
leptons as a fourth colour and is rank-4, not rank-3. The
extension to $U(1)_X$ in the broken gauge group is the
substrate-physical signature of the lepton-number-as-fourth-colour
$U(1)$ that survives Wilson-line breaking; it is anomaly-free in
the BHOP construction.
...
\S 6 introduces the rank-4 $SU(4)$ extension construction used as the
framework's bundle commitment; Eq.~99 records the heterotic Bianchi
anomaly-cancellation identity with the positive-tension $5$-brane
wrapping $\tau_1^2$.
...
``lepton number as the fourth colour'' reading of the rank-4 $SU(4)$
internal-symmetry structure committed in this paper.
