...
The construction is a strong-selection structural identification, not yet a forced derivation: the substrate-physical projection onto the heterotic moduli of $X/(\mathbb{Z}_3 \times \mathbb{Z}_3)$ is not closed in this paper. The $\sigma$-discrimination Schoen-vs-Tian--Yau result is reported as a four-tier transparency table (conservative full $20{+}20$ seeds: $|t| = 3.99\sigma$, percentile~95\% CI $[3.29, 10.64]$ -- referee-safe unconditional reading, CI lower bound straddles $5\sigma$; strict-converged tier: $|t| = 6.92\sigma$, BCa $[5.30, 9.04]$ -- conditional on the convergence filter that preferentially trims $4/20$ Schoen seeds from the upper tail; bundle-construction-specific) and is reported as supporting evidence pending the converged PDG-fermion-mass Yukawa-cohomology pipeline. Dark-sector chirality assignments and several closure-rule numerators are encoded structural-rule-consistent choices whose generative derivation is deferred.
\end{abstract}
...
normalization factor, structurally defined as $(\dim - 2)/(2\dim)$
--- an arithmetic gcd-reduction of an $E_8$ algebraic invariant, not
an empirical eigenvalue. The $h_{\rm eff} = m_e \cdot 124/123$
identity is therefore a definitional rearrangement once $m_e$ is
supplied, and its empirical content is the muon-formula
self-consistency
$h_{\rm eff} = m_\mu / R_\mu$ at $\sim 30$~ppb (test of $R_\mu$
given $m_e$, not of $\omega_{\rm fix}$). Equivalently, with the
Planck mass
...
discrimination.} The Schoen-vs-TY $\sigma$-channel result
(reported as a four-tier table in
Sec.~\ref{sec:cy3-substrate-identification}: conservative tier
$|t| = 3.99\sigma$, full $20{+}20$ seeds, percentile~95\% CI
$[3.29, 10.64]$ — CI lower bound straddles $5\sigma$;
strict-converged tier $|t| = 6.92\sigma$, BCa $[5.30, 9.04]$,
conditional on the convergence filter trimming $4/20$ Schoen
seeds preferentially from the upper tail) depends on the
specific choice of Donaldson h-inversion procedure and the
Kähler-form projection convention used in the metric pipeline.
The paired-bootstrap sign-reversal defense addresses the
basis-size artifact at matched basis size; it does not address
convention-choice artifacts. \emph{Required work:} robustness
sweep over $\geq 3$ published convention variants for both the
h-inversion handling and the Kähler-form projection, with a
stress-test report on whether the four-tier result survives or
degrades. The $\sigma$-channel discrimination should be
re-stated with an explicit convention sensitivity envelope on
both the conservative and strict tiers.
...
\paragraph{Path A research closure (verdict (a) at the audited scope).}
At the time of writing, the Path A heterotic line of attack has reached
closure \emph{within an explicitly audited scope}: the line-bundle
shadow + $W$-twisted SES on the $\mathbb{Z}_3 \times \mathbb{Z}_3$-invariant
subspace at the BHOP-canonical bundle-frame lift
$(g_\alpha, g_\beta) = (0, 0)$. Three sequential computational
artefacts on the \texttt{Schoen}~$\widetilde X / \Gamma$ quotient
establish:
...
\item \textbf{Shadow $\mathrm{Ext}^1 = 0$.} The line-bundle-shadow
  ...
  \path{references/p_ext1_engagement_2026-05-05.md}). The shadow
  truncation drops the rank-2 dP9 bundle $W$ carried by the full
  $V_2 = \mathcal{O}(\tau_1 - \tau_2) \otimes \pi_2^*(W)$; this caveat
  is self-disclosed in
  \path{src/bin/p_ext1_compute.rs} lines 19--37, 87--90, 324--330.
...
\item \textbf{$\mathbb{Z}_3 \times \mathbb{Z}_3$-invariant projection
  yields $0$ at the $(0,0)$ lift.} The $+18$ resides entirely in the
  non-invariant $\beta = 2$ characters; the BHOP-canonical bundle-frame
  lift $(g_\alpha, g_\beta) = (0, 0)$ projects the invariant subspace
  to exactly $0$ at both the lower and upper SES bounds (commit
  \texttt{a4e6231e};
  \path{references/p_dp9_w_invariant_proj_2026-05-06.md}).
\end{itemize}
Within that scope this chain of three computations settles
$\mathrm{Ext}^1_{\mathrm{shadow}} = 0$ on the Schoen
$\mathbb{Z}_3 \times \mathbb{Z}_3$ quotient. The robustness argument
is geometric: the BHOP \S 6.1 SES sub-bundle $\mathcal{O}(-2f)$ rigs
the elliptic-fiber $H^1$ direction to sit at $\beta = 2 \neq 0$, and
no admissible choice of $L_\mathrm{base}$ on dP9 can move that
character without breaking the SES design (see the cited reference
for the closed-form proof). \textbf{Scope caveat.} A complete
proof of $\mathrm{Ext}^1_{\mathrm{full}} = 0$ across all bundle-frame
lifts $(g_\alpha, g_\beta) \in (\mathbb{Z}_3)^2$ and beyond the
line-bundle shadow truncation requires dP9-cohomology of the rank-2
$W$-bundle in its own right; the present chain proves vanishing on
the audited scope only.
...
Post Path A closure at the audited scope (verdict (a),
line-bundle-shadow $+$ $W$-twisted SES on the
$\mathbb{Z}_3 \times \mathbb{Z}_3$-invariant subspace, $(0,0)$ lift),
the Yukawa matrices emitted by the assembly binary at the rank-4
BHOP-2005 $\kappa = 0$ fixed point are the framework's
\emph{canonical prediction at that scope} for the
...
\begin{itemize}
\item \textbf{electron at $k = 0$} -- gateway-normalisation
coefficient $\omega_{\rm fix} = (\dim - 2)/(2\dim) = 123/248$, an
exact $E_8$ algebraic identity (P7.12), structurally defined and not
an empirical eigenvalue. Off the Coxeter ladder; no chain-position
