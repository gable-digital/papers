...
- **σ-discriminability is not a likelihood.** The σ-channel is reported as a separate diagnostic |t|-statistic (4-tier table: 3.99σ conservative full 20+20 / 6.92σ strict-converged conditional, BCa [5.30, 9.04]; conservative-tier CI lower bound straddles 5σ at 3.29; see `cy3_solver/references/cy3_publication_summary.md` §1), not as a Bayes-factor likelihood contribution, because of the basis-size sign-reversal documented in Track 2. Reviewers should not interpret either σ tier as additive to the 5.43σ chain-channel result.
